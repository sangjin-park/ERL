...
class Agent(BaseModel):
  def __init__(self, config, environment, sess, ae):
    ...
    self.ae_screen_shape = (self.ae_screen_height, self.ae_screen_width)
    ...
    for self.step in tqdm(list(range(start_step, self.max_step)), ncols=70, initial=start_step):
      ...
      if terminal:
        ...
        if self.step > self.ae_start:
          ep_transition_loss_plus_ratio = []
          ep_length = len(self.ep_transition_cache)



          for i in range(0, ep_length, self.batch_size):
            data = np.array([imresize(x[0], self.ae_screen_shape) for x in self.ep_transition_cache[i:min(ep_length, i+self.batch_size)]])
            data = data.reshape(data.shape+(1,))
            loss_plus_ratio = self.ae.evaluate_sample(data, "ep")
            # loss_plus_ratio = self.ae.evaluate_sample(data/255.)
            ep_transition_loss_plus_ratio += list(loss_plus_ratio)

          # backward smooth.
          # idx = ep_length
          # while idx > 1:
          #   idx -= 1
          #   if ep_transition_loss_plus_ratio[idx] == 0 or ep_transition_loss_plus_ratio[idx-1] > \
          #           ep_transition_loss_plus_ratio[idx]:
          #     continue
          #   # smooth from the latter frame
          #   ep_transition_loss_plus_ratio[idx-1] = 0.5 * ep_transition_loss_plus_ratio[idx-1] + \
          #                                          0.5 * ep_transition_loss_plus_ratio[idx]

#          Perfrom the Cut
 #         print ep_transition_loss_plus_ratio
 #          ep_cuts = self.cut(ep_transition_loss_plus_ratio)
 #          ep_cut_length = 0
 #          if self.log_ep % 50 == 0:
 #            ep_save = True
 #          else:
 #            ep_save = False
          ...
          # for c in ep_cuts:
          #   ep_cut_length += c[1]
          #   for i in range(c[0], c[0]+c[1]-1):
          #     if ep_save:
          #       imsave("./tmp/ep%i_cut%i_%f.jpg" % (self.log_ep, i, ep_transition_loss_plus_ratio[i]), self.ep_transition_cache[i][0])
          #     self.memory.add(*self.ep_transition_cache[i])
          #
          #   self.memory.add(*self.ep_transition_cache[c[0]+c[1]-1], cut=True)
          #
          # print "ep_cuts:%i, ep_cut_lengthL%i, ep_length:%i" % (len(ep_cuts), ep_cut_length, ep_length)
          # self.log_ep += 1

        # if self.step - self.last_play > self.play_interval:
        #   self.play(test_ep=0, render=False)
        #   self.last_play = self.step
  ...
  def cut(self, loss_ratio_plus):
    ep_cuts = []
    ep_length = len(loss_ratio_plus)
    s, l, idx = 0, 0, 0
    while idx < ep_length:
      if loss_ratio_plus[idx] > self.ae_threshold:
        if l == 0:
          s = idx
        l += 1
      else:
  ...
  def observe(self, screen, reward, action, terminal):
    ...
    if not self.step > self.ae_start:
      self.memory.add(screen, reward, action, terminal) # its initial state
    else:
      self.ep_transition_cache.append((screen, reward, action, terminal))
    ...
    if self.step > self.ae_learn_start:
      if self.step % self.train_frequency == 0:
        batch = self.ae.memory.sample()
        self.ae.ae_train_mini_batch(batch)
  ...
  def q_learning_mini_batch(self):
    ...
    # TODO modify the rewards
    # the shape of them are 32 84 84 4

    if self.step > self.ae_start:
      ae_data = np.array([imresize(s_t_plus_1[x,:,:,-1], self.ae_screen_shape) for x in range(self.batch_size)])
      # ae_data = np.array([imresize(s_t_plus_1[x,:,:,-1], self.ae_screen_shape) for x in range(self.batch_size)])/255.
      bonus = self.ae.evaluate_sample(ae_data.reshape(ae_data.shape+(1,))) - self.ae_threshold
      bonus[bonus>0] = 0.005
      reward = np.minimum(reward + bonus, 1)
