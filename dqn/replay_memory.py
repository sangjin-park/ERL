...
class ReplayMemory:
  def __init__(self, config, model_dir):
    ...
    self.cuts = np.empty(self.memory_size, dtype = np.bool)
  ...
  def add(self, screen, reward, action, terminal, cut = False):
    ...
    while len(indexes) < self.batch_size:
      ...
      while True:
        ...
        if self.terminals[(index - self.history_length):index].any() or \
                self.cuts[(index - self.history_length):index+1].any():
          continue
